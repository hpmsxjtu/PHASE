...
public class Schedule {
	 ...
	 */
	public def schedule_event(start:double, fun:Functor):ScheduledEvent{
		/**
	 ...
	 * 						that wrapped the object's action.
	 */
	public def schedule_event(start:double, interval:double, fun:Functor):ScheduledEvent{
		/**
		 * Create a new SimEvent with start and fun.
		 */
		val evt:ScheduleSimEvent = new ScheduleSimEvent();
	...
	public def schedule_stop_event(start:double, fun:Functor):ScheduledEvent{
		/**
		 * Create a new SimEvent with start and fun.
		 */
		val evt: ScheduleSimEvent = new ScheduleSimEvent();
	...
	public def execute():void{
		if(!queue.empty()){
			...
			//gjm_addcode
			// Console.OUT.println("currentTick : "+currentTick+"   Place:"+here.id);
			val fun: Functor = event.getEvent().getFun();
			fun();        
			event.reschedule(queue);
			queue.pop();
	...
	public def getNextTick():double{
		if(queue.empty()||(queue.top().getEvent().getTick())==EndTick){
			return -1.0;
		}else{
